...
def EventDisplay( tubes, quantities, PMTFlatMapPositive, title="Charge", cutrange=[-1,-1], cmap = None ):
    """
    ...
    im = ax.imshow( preimage, extent = [-positive_x_offset,positive_x_offset,-lower_endcap_offset,lower_endcap_offset], vmin=imgmin, vmax=imgmax, cmap = cmap )
...
def EventSubsetDisplay( tubes, quantities, PMTFlatMapPositive, tubes_to_plot, title="Charge", cutrange=[-1,-1], padding=10, cmap = None):
    """
    tubes == np.array of PMTs that were hit
    quantities == np.array of PMT quantities (either charge or time)
    title == title to add to display
    ...
    im = ax.imshow( subset_image, extent = [min_subplot_x_value, max_subplot_x_value, min_subplot_y_value, max_subplot_y_value], vmin=imgmin, vmax=imgmax, cmap = cmap )
